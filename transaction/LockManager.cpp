...
LockManager::LockManager()
  ...
  , detector_status_(
        std::make_unique<DeadLockDetectorStatus>(
            DeadLockDetectorStatus::kNOT_READY))
  , victim_result_(std::make_unique<std::vector<TransactionId>>()) {
  deadlock_detector_thread_
      = std::make_unique<DeadLockThread>(lock_table_.get(),
...
bool LockManager::releaseAllLocks(TransactionId tid) {
  ...
  if (transaction_deleted
          == TransactionTableResult::kTRANSACTION_DELETE_ERROR) {
...
void LockManager::killVictims() {
  ...
  if (*detector_status_ == DeadLockDetectorStatus::kDONE) {
    for (std::vector<TransactionId>::const_iterator iter
