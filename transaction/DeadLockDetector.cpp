...
DeadLockDetector::getAllDependents(TransactionId owner) {
  FATAL_ERROR("Not implemented");
}

std::vector<TransactionId>
DeadLockDetector::getAllDependees(TransactionId pending) {
  DepGraph::NodeId pending_node_id = getNodeId(pending);
  std::vector<DepGraph::NodeId> nodes
...
DeadLockDetector::DepGraph::NodeId
DeadLockDetector::getNodeId(TransactionId tid) {
...
DeadLockDetector::DepGraph::NodeId
DeadLockDetector::addNode(TransactionId tid) {
...
std::vector<TransactionId> DeadLockDetector::getAllVictims() {
  ...
  for (LockTable::ConstIterator it = lock_table_->begin();
    ...
    for (LockTable::LockOwnList::const_iterator it_own_list = own_list.begin();
      ...
      for (LockTable::LockPendingList::const_iterator
