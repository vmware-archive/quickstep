...
WorkOrder* WorkOrderFactory::ReconstructFromProto(const serialization::WorkOrder &proto,
  ...
  switch (proto.work_order_type()) {
    case serialization::MOCK: {
      return new MockWorkOrder(proto.GetExtension(serialization::MockWorkOrder::operator_index));
    }
    ...
    case serialization::BUILD_HASH: {
      ...
      return new BuildHashWorkOrder(
          *catalog_database->getRelationById(
    ...
    case serialization::SORT_MERGE_RUN: {
      ...
      std::vector<merge_run_operator::Run> runs;
      ...
      return new SortMergeRunWorkOrder(
          ...
          *query_context->getSortConfig(
              proto.GetExtension(serialization::SortMergeRunWorkOrder::sort_config_id)),
          move(runs),
    ...
    case serialization::SORT_RUN_GENERATION: {
      ...
      return new SortRunGenerationWorkOrder(
          ...
          *query_context->getSortConfig(
              proto.GetExtension(serialization::SortRunGenerationWorkOrder::sort_config_id)),
    ...
    case serialization::REBUILD: {
      LOG(INFO) << "Creating RebuildWorkOrder";
      const relation_id rel_id = proto.GetExtension(serialization::RebuildWorkOrder::relation_id);

      MutableBlockReference block_ref(
          storage_manager->getBlockMutable(proto.GetExtension(serialization::RebuildWorkOrder::block_id),
                                           *catalog_database->getRelationById(rel_id)));
      return new RebuildWorkOrder(move(block_ref),
                                  proto.GetExtension(serialization::RebuildWorkOrder::operator_index),
                                  rel_id,
                                  foreman_client_id,
                                  bus);
    }
...
bool WorkOrderFactory::ProtoIsValid(const serialization::WorkOrder &proto,
  ...
  switch (proto.work_order_type()) {
    case serialization::MOCK: {
      // NOTE(zuyu): serialization::MockWorkOrder::id is not used by Foreman.
      return proto.HasExtension(serialization::MockWorkOrder::operator_index);
    }
    case serialization::AGGREGATION: {
    ...
    case serialization::BUILD_HASH: {
      ...
      const CatalogRelation *relation = catalog_database.getRelationById(rel_id);
      for (int i = 0; i < proto.ExtensionSize(serialization::BuildHashWorkOrder::join_key_attributes); ++i) {
    ...
    case serialization::DELETE: {
      return proto.HasExtension(serialization::DeleteWorkOrder::relation_id)
          ...
          && query_context.isValidPredicateId(
                 proto.GetExtension(serialization::DeleteWorkOrder::predicate_index))
    ...
    case serialization::HASH_JOIN: {
      ...
      const CatalogRelation *build_relation = catalog_database.getRelationById(build_relation_id);
      const CatalogRelation *probe_relation = catalog_database.getRelationById(probe_relation_id);
      for (int i = 0; i < proto.ExtensionSize(serialization::HashJoinWorkOrder::join_key_attributes); ++i) {
    ...
    case serialization::SORT_MERGE_RUN: {
      return proto.HasExtension(serialization::SortMergeRunWorkOrder::sort_config_id)
          && query_context.isValidSortConfigId(
                 proto.GetExtension(serialization::SortMergeRunWorkOrder::sort_config_id))
          && proto.HasExtension(serialization::SortMergeRunWorkOrder::runs)
    ...
    case serialization::SORT_RUN_GENERATION: {
      return proto.HasExtension(serialization::SortRunGenerationWorkOrder::relation_id)
          ...
          && query_context.isValidInsertDestinationId(
                 proto.GetExtension(serialization::SortRunGenerationWorkOrder::insert_destination_index))
          && proto.HasExtension(serialization::SortRunGenerationWorkOrder::sort_config_id)
          && query_context.isValidSortConfigId(
                 proto.GetExtension(serialization::SortRunGenerationWorkOrder::sort_config_id))
    ...
    case serialization::REBUILD: {
      return proto.HasExtension(serialization::RebuildWorkOrder::relation_id)
          && catalog_database.hasRelationWithId(
                 proto.GetExtension(serialization::RebuildWorkOrder::relation_id))
          && proto.HasExtension(serialization::RebuildWorkOrder::block_id);
    }
    default:
      return false;
  }
