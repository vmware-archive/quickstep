...
class UpdateWorkOrder : public WorkOrder {
 public:
   ...
   * @brief Constructor
   *
   * @param input_relation_schema The relation schema to perform the UPDATE
   *        over.
   * @param input_block_id The block id.
   * @param update_assignments The update group (the map of attribute_ids to
   *        Scalars) which should be evaluated to get the new value for the
   *        corresponding attribute.
   * @param update_operator_index The index of the Update Operator in the query
   *        plan DAG.
   * @param foreman_client_id The TMB client ID of the Foreman thread.
   * @param predicate All tuples matching \c predicate will be updated (or
   *        NULL to update all tuples).
   * @param relocation_destination Where to relocate tuples which can not be
   *        updated in-place.
   * @param storage_manager The StorageManager to use.
   * @param bus A pointer to the TMB.
   **/
  UpdateWorkOrder(const CatalogRelationSchema &input_relation_schema,
                  const block_id input_block_id,
                  const std::unordered_map<attribute_id, std::unique_ptr<const Scalar>> &update_assignments,
                  ...
                  tmb::MessageBus *bus)
      : input_relation_schema_(input_relation_schema),
        input_block_id_(input_block_id),
        update_assignments_(update_assignments),
 ...
 private:
  const CatalogRelationSchema &input_relation_schema_;
  const block_id input_block_id_;
  const std::unordered_map<attribute_id, std::unique_ptr<const Scalar>> &update_assignments_;
