...
void Regexp::AddRuneToString(Rune r) {
  ...
  if (lit_str_.nrunes_ == 0) {
    // start with 8
    lit_str_.runes_ = new Rune[8];
  } else if (lit_str_.nrunes_ >= 8 && (lit_str_.nrunes_ & (lit_str_.nrunes_ - 1)) == 0) {
    // double on powers of two
    Rune *old = lit_str_.runes_;
    lit_str_.runes_ = new Rune[lit_str_.nrunes_ * 2];
    for (int i = 0; i < lit_str_.nrunes_; i++)
      lit_str_.runes_[i] = old[i];
    delete[] old;
  }

  lit_str_.runes_[lit_str_.nrunes_++] = r;
}
  ...
  switch (re->op_) {
    ...
    case kRegexpLiteralString:
      ...
      if (re->parse_flags() & Latin1) {
        prefix->resize(re->lit_str_.nrunes_);
        for (int j = 0; j < re->lit_str_.nrunes_; j++)
          (*prefix)[j] = static_cast<char>(re->lit_str_.runes_[j]);
      } else {
        // Convert to UTF-8 in place.
        // Assume worst-case space and then trim.
        prefix->resize(re->lit_str_.nrunes_ * UTFmax);
        char *p = &(*prefix)[0];
        for (int j = 0; j < re->lit_str_.nrunes_; j++) {
          Rune r = re->lit_str_.runes_[j];
          if (r < Runeself)
