...
TEST_F(AggregateTest, BasicTest) {
  for (bool is_distinct : {true, false}) {
    const E::AggregateFunctionPtr sum = createSum(add_literal_0_, true, is_distinct);
...
TEST_F(AggregateTest, PullupSelection) {
  for (bool is_distinct : {true, false}) {
    const E::AggregateFunctionPtr sum = createSum(E::ToRef(alias_add_literal_0_), true, is_distinct);
    const E::AliasPtr sum_alias = createAlias(sum,
                                              "sum" /* attribute_name */,
                                              "test_relation" /* relation_name */);
