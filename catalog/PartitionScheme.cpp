...
PartitionScheme* PartitionScheme::ReconstructFromProto(const serialization::PartitionScheme &proto) {
  ...
  const serialization::Partitions &proto_partitions = proto.blocks_in_partitions();
  for (int i = 0; i < proto_partitions.partitions_size(); ++i) {
    unordered_set<block_id> blocks;

    const serialization::Partition &proto_blocks = proto_partitions.partitions(i);
...
serialization::PartitionScheme PartitionScheme::getProto() const {
  ...
  proto.mutable_header()->MergeFrom(move(header_->getProto()));

  // Add blocks to the corresponding partitions.
  for (std::size_t i = 0; i < blocks_in_partition_.size(); ++i) {
    serialization::Partition *proto_blocks = proto.mutable_blocks_in_partitions()->add_partitions();
...
partition_id PartitionScheme::getPartitionForBlock(
