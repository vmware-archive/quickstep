...
class CallData {
 public:
  CallData(DataExchange::AsyncService *service,
...
void CallData::Proceed() {
  switch (status_) {
    ...
    case CallStatus::PROCESS: {
      ...
      new CallData(service_, queue_, storage_manager_);
...
const char *DataExchangerAsync::server_address_ = "0.0.0.0:";
...
DataExchangerAsync::DataExchangerAsync() {
  ...
  LOG(INFO) << "DataExchangerAsync Service listening on " << server_address_ << port_;
}

void DataExchangerAsync::run() {
  DCHECK(storage_manager_ != nullptr);

  // Delete by itself upon success.
  new CallData(&service_, queue_.get(), storage_manager_);
